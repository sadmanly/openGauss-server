...
DROP TABLE IF EXISTS public.embedding_hnsw_exchange_parent_no_stage_index_script CASCADE;
DROP TABLE IF EXISTS public.embedding_hnsw_exchange_stage_no_stage_index_script CASCADE;

\echo ==== Step 2: Create parent table with two partitions ====
CREATE TABLE public.embedding_hnsw_exchange_parent_no_stage_index_script (
    id int,
    part_key int,
    title text,
    embedding vector(3)
...
INSERT INTO public.embedding_hnsw_exchange_parent_no_stage_index_script VALUES
    (1, 1, 'seed-1', '[1,2,3]'),
    (2, 2, 'seed-2', '[2,3,4]'),
...
FROM public.embedding_hnsw_exchange_parent_no_stage_index_script t
JOIN pg_partition p ON p.oid = t.tableoid
GROUP BY p.relname
ORDER BY p.relname;

\echo ==== Step 3: Create LOCAL HNSW index on parent table ====
CREATE INDEX embedding_hnsw_exchange_parent_no_stage_index_idx
ON public.embedding_hnsw_exchange_parent_no_stage_index_script
...
SELECT pg_get_indexdef('public.embedding_hnsw_exchange_parent_no_stage_index_idx'::regclass, true);

SELECT 'parent_index_parts_initial' AS check_name, relname, indisusable
FROM pg_partition
WHERE parentid = 'public.embedding_hnsw_exchange_parent_no_stage_index_idx'::regclass
ORDER BY relname;

\echo ==== Step 4: Create stage table without vector index ====
CREATE TABLE public.embedding_hnsw_exchange_stage_no_stage_index_script (
    id int,
    part_key int,
    title text,
    embedding vector(3)
...
FROM pg_index
WHERE indrelid = 'public.embedding_hnsw_exchange_stage_no_stage_index_script'::regclass;

\echo ==== Step 5: COPY 100 rows into stage table ====
COPY public.embedding_hnsw_exchange_stage_no_stage_index_script (id, part_key, title, embedding)
...
FROM public.embedding_hnsw_exchange_stage_no_stage_index_script;

\echo ==== Step 6: Add new partition p3 to parent table ====
ALTER TABLE public.embedding_hnsw_exchange_parent_no_stage_index_script
...
SELECT 'stage_index_count_before_exchange' AS check_name, count(*) AS idx_count
FROM pg_index
WHERE indrelid = 'public.embedding_hnsw_exchange_stage_no_stage_index_script'::regclass;
...
\echo ==== Step 7: Expected failure when stage table has no index ====
\set ON_ERROR_STOP off
ALTER TABLE public.embedding_hnsw_exchange_parent_no_stage_index_script
EXCHANGE PARTITION (p3) WITH TABLE public.embedding_hnsw_exchange_stage_no_stage_index_script;
\set ON_ERROR_STOP on

\echo ==== Step 8: Validate state after failed EXCHANGE ====
SELECT 'parent_rows_after_failed_exchange' AS check_name, p.relname AS partition_name, count(*) AS row_count
FROM public.embedding_hnsw_exchange_parent_no_stage_index_script t
JOIN pg_partition p ON p.oid = t.tableoid
GROUP BY p.relname
ORDER BY p.relname;

SELECT 'stage_rows_after_failed_exchange' AS check_name, count(*) AS row_count
FROM public.embedding_hnsw_exchange_stage_no_stage_index_script;

SELECT 'stage_index_count_after_failed_exchange' AS check_name, count(*) AS idx_count
FROM pg_index
WHERE indrelid = 'public.embedding_hnsw_exchange_stage_no_stage_index_script'::regclass;
...
-- gsql:/home/s00524258/openGauss-server/verify_hnsw_exchange_without_stage_index.sql:8: NOTICE:  table "embedding_hnsw_exchange_parent_no_stage_index_script" does not exist, skipping
-- DROP TABLE
-- gsql:/home/s00524258/openGauss-server/verify_hnsw_exchange_without_stage_index.sql:9: NOTICE:  table "embedding_hnsw_exchange_stage_no_stage_index_script" does not exist, skipping
-- DROP TABLE
...
--                                                                                                             pg_get_indexdef                                                                                                             
-- ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--  CREATE INDEX embedding_hnsw_exchange_parent_no_stage_index_idx ON embedding_hnsw_exchange_parent_no_stage_index_script USING hnsw (embedding vector_l2_ops) LOCAL(PARTITION p1_hnsw_idx, PARTITION p2_hnsw_idx)  TABLESPACE pg_default
-- (1 row)
-- 
...
-- (1 row)
-- 
-- ==== Step 6: Add new partition p3 to parent table ====
-- ALTER TABLE
--              check_name             |     relname      | indisusable 
-- ------------------------------------+------------------+-------------
...
--             check_name             | idx_count 
-- -----------------------------------+-----------
--  stage_index_count_before_exchange |         0
-- (1 row)
-- 
...
-- (1 row)
-- 
-- ==== Step 7: Expected failure when stage table has no index ====
-- gsql:/home/s00524258/openGauss-server/verify_hnsw_exchange_without_stage_index.sql:195: ERROR:  tables in ALTER TABLE EXCHANGE PARTITION must have the same number of indexs
-- ==== Step 8: Validate state after failed EXCHANGE ====
--             check_name             | partition_name | row_count 
-- -----------------------------------+----------------+-----------
--  parent_rows_after_failed_exchange | p1             |         2
--  parent_rows_after_failed_exchange | p2             |         2
-- (2 rows)
-- 
--             check_name            | row_count 
-- ----------------------------------+-----------
--  stage_rows_after_failed_exchange |       100
-- (1 row)
-- 
--                check_name                | idx_count 
-- -----------------------------------------+-----------
--  stage_index_count_after_failed_exchange |         0
-- (1 row)
-- 
-- total time: 501  ms
